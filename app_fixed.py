...
# In the instructor_assignments route, replace the problematic line:
...
@app.route('/api/instructor_assignments', methods=['GET'], endpoint='instructor_assignments_2')
def instructor_assignments():
    if 'user_id' not in session or session.get('role') != 'instructor':
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    for assignment in all_assignments:
        ...
        assignments_data.append({
            ...
            'total_submissions': total_submissions,
            'passed_submissions': passed_submissions,
            'test_cases': json.loads(assignment.test_cases or '[]') # Ensure test_cases is a list, even if empty
        })
